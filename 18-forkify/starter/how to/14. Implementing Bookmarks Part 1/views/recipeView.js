...
class RecipeView extends View {
  ...
  // TODO eventHandlersUpdateServings
  addHandlerUpdateServings(handler) {
    // * Event delegation closest element
    this._parentElement.addEventListener('click', function (e) {
      const btn = e.target.closest('.btn--update-servings');
      if (!btn) return;
      // const updateTo = +btn.dataset.updateTo; // HACK + converts string to number
      const { updateTo } = btn.dataset; //HACK Destructuring
      if (+updateTo > 0) handler(+updateTo);
    });
  }

  // TODO eventHandlerAddBookmark
