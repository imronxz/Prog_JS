...
export const loadRecipe = async function (id) {
  try {
    ...
    const data = await AJAX(`${API_URL}/${id}?key=${KEY_API_URL}`);
...
export const loadSearchResults = async function (query) {
  try {
    ...
    const data = await AJAX(`${API_URL}?search=${query}&key=${KEY_API_URL}`);
    console.log(data);
...
export const uploadRecipe = async function (newRecipe) {
  ...
  try {
    ...
    const data = await AJAX(`${API_URL}?key=${KEY_API_URL}`, recipe);
    state.recipe = createRecipeObject(data);
